...
function TheSinhVienList() {
  const danhSachSinhVien = [
    { ten: "Đào Mạnh Dũng", tuoi: 20, lop: "B24DCCC077" },
  ...
  return (
    <div>
      {danhSachSinhVien.map((sv, index) => (
        <TheSinhVien key={index} ten={sv.ten} tuoi={sv.tuoi} lop={sv.lop} />
      ))}
    </div>
  );
}

export default TheSinhVienList;
